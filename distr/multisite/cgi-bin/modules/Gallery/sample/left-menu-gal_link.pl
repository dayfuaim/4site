...
my $cat = $q->param('cat') || $sitemodules::DBfunctions::dbh->selectrow_array("SELECT gallerycategory_id, gallerycategory_fld FROM gallerycategory_tbl WHERE page_id=$current_ID AND enabled_fld='1' LIMIT 1");
my $curnum = (defined $q->param('curnum'))?$q->param('curnum'):0;
